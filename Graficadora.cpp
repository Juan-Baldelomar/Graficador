...
//    cairo_fill(cr);
    ...
    cairo_move_to(cr, origenx, origeny); cairo_line_to(cr, origenx, endy);
    cairo_move_to(cr, origenx, endy); cairo_line_to(cr, endx, endy);
    ...
    if (a<=0 && b>=0){
        ...
        cairo_move_to(cr, ejex, origeny); cairo_line_to(cr, ejex, endy);
    }
    ...
    if (fMin <= 0 && fMax>= 0){
       // graficar eje x 
        ...
        cairo_move_to(cr, origenx, ejey); cairo_line_to(cr, endx, ejey);
    ...
    for (int i = 0; i < n; i++) {
        ...
        double valy = endy - (f(valx)-fMin)/(fMax-fMin)*(endy-origeny);
